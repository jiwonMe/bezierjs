...
var { abs, min, max, cos, sin, acos, atan2, sqrt, pow } = Math;
function crt(v) {
...
var derive = function(points, _3d) {
  const dpoints = [];
...
var between = function(v, m, M) {
  return m <= v && v <= M || approximately(v, m) || approximately(v, M);
};
var approximately = function(a, b, precision) {
...
var closest = function(LUT, point) {
...
// src/utils/roots.js
var roots = function(points, line) {
...
var inflections = function(points) {
  if (points.length < 4)
    return [];
...
var expandbox = function(bbox, _bbox) {
...
var pairiteration = function(c1, c2, curveIntersectionThreshold) {
...
var getccenter = function(p1, p2, p3) {
  const dx1 = p2.x - p1.x, dy1 = p2.y - p1.y, dx2 = p3.x - p2.x, dy2 = p3.y - p2.y, dx1p = dx1 * cos(quart) - dy1 * sin(quart), dy1p = dx1 * sin(quart) + dy1 * cos(quart), dx2p = dx2 * cos(quart) - dy2 * sin(quart), dy2p = dx2 * sin(quart) + dy2 * cos(quart), mx1 = (p1.x + p2.x) / 2, my1 = (p1.y + p2.y) / 2, mx2 = (p2.x + p3.x) / 2, my2 = (p2.y + p3.y) / 2, mx1n = mx1 + dx1p, my1n = my1 + dy1p, mx2n = mx2 + dx2p, my2n = my2 + dy2p, arc = lli8(mx1, my1, mx1n, my1n, mx2, my2, mx2n, my2n), r = dist(arc, p1);
...
var findbbox = function(sections) {
...
var shapeintersections = function(s1, bbox1, s2, bbox2, curveIntersectionThreshold) {
...
// src/utils/shape.js
var BezierClass = null;
function setBezierClass(Bezier2) {
  BezierClass = Bezier2;
}
...
var makeshape = function(forward, back, curveIntersectionThreshold) {
...
var lookupMethods = {
  ...
  extrema() {
    ...
    result.values = roots2.sort(utils.numberSort).filter(function(v, idx) {
...
var geometryMethods = {
  ...
  split(t1, t2) {
    ...
    const result = {
      left: this.order === 2 ? new this.constructor([q[0], q[3], q[5]]) : new this.constructor([q[0], q[4], q[7], q[9]]),
      right: this.order === 2 ? new this.constructor([q[5], q[4], q[2]]) : new this.constructor([q[9], q[8], q[6], q[3]]),
      span: q
    };
...
// src/bezier/intersection.js
var intersectionMethods = {
  intersects(curve, curveIntersectionThreshold) {
    ...
    if (curve.constructor.name === "Bezier" || curve instanceof this.constructor) {
      curve = curve.reduce();
    }
...
// src/bezier.js
var Bezier = class extends BezierCore {
  constructor(...args) {
    super(...args);
  }
};
